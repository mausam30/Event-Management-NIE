...
class EventTableGateway {
    ...
    public function getEvents() {
        ...
        $sqlQuery = "SELECT e.*, l.name " .
                    ...
                    //NEW
                    // $sqlQuery2 = "SELECT e.*, l.link " .
                    // "FROM events e " .
                    // "LEFT JOIN locations l ON e.EventID = l.EventID";
        
        $statement = $this->connect->prepare($sqlQuery);
        // $statement2 = $this->connect->prepare($sqlQuery2);
    ...
    public function getEventsByLinkId($id) {
        // execute a query to get all events
        $sqlQuery = "SELECT e.*, l.link " .
                    "FROM events e " .
                    "LEFT JOIN links l ON e.EventID = l.EventID " .
                    "WHERE e.EventID=:EventID";
        
        $params = array(
            "EventID" => $id
        );
        $statement = $this->connect->prepare($sqlQuery2);
        $status = $statement->execute($params);
    ...
    public function insert($p) {
        $sql = "INSERT INTO events(Title, Description, StartDate, EndDate,StartTime,EndTime,  LocationID) " .
                "VALUES (:Title, :Description, :StartDate, :EndDate,:StartTime,:EndTime, :LocationID)";
